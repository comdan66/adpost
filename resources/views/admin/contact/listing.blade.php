						...
						<table class="table table-striped table-bordered table-hover dataTable">
							...
							<thead>
								<tr>
									...
									<th style="width:auto" class="sortable" v-on:click="changeOrder('email')" orderable>Email</th>
									<th style="width:200px" class="sortable" v-on:click="changeOrder('phone')" orderable>電話</th>
								...
								<tr>
									...
									<th>
									<input ref="searchInput" v-on:keyup.enter="inputSearch()" v-model="search.phone" class="form-control" />
									</th>
									<!--
									<th>

									<select class="form-control" v-model="search.isContact">
									<option value="">----</option>
									...
									<select class="form-control" v-model="search.isRead">
									<option value="">----</option>
									<option v-for="(item, index) of option.is" v-bind:value="index">@{{ item }}</option>
									</select>
									</th> -->
							...
							<tbody>
								<tr v-for="(item, index) of items">
									...
									<td>@{{ item.email }}</td>
									<td>@{{ item.phone }}</td>
									<!-- <td>@{{ option.is[item.isContact] }}</td> -->
									<!-- <td>@{{ option.is[item.isRead] }}</td> -->
