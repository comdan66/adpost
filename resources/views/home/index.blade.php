...
<div class="kv-home row visible-xs visible-sm">
	<div class="col-xs-12">
		...
		<div class="bt-action col-xs-6 center-block">
			<a class="btn btn-post" href="/login" role="button">馬上註冊</a>
		</div>
	</div>
</div>
...
<script>
	setTimeout("showVideo('new');showPhoto('new');", 1500);
	...
	*/
	...
	function showPhoto(x) {
		...
		if (x == 'views') {
			if (p1 == null) {
				p1 = jQuery('#p1').bxSlider(bxSliderOption);
			}
		}
		if (x == 'popular') {
			if (p2 == null) {
				p2 = jQuery('#p2').bxSlider(bxSliderOption);
			}
		}
	...
	// p0 = jQuery('#p0').bxSlider(bxSliderOption);

	jQuery(document).ready(function() {

		// alert('sa');

		// setTimeout("showVideo('new');showPhoto('new');alert('zzz')", 500);
		showVideo('new');
		showPhoto('new');
