...
class AdLikeController  extends _ControllerAdmin {
	...
	public function getList() {
		...
		if (strpos($orderField, '.') === false) {
			$orderField = 'ad_like.' . $orderField;
		}

		$result = AdLike::join('user', 'user.id', 'ad_like.userID') -> orderby($orderField, $orderType);

		addWhere($result, 'id', '=', 'ad_like.id');
		addWhere($result, 'adID', '=', 'ad_like.adID');
		...
		$select = array('ad_like.*', 'user.email', 'user.name as userName');
	...
	/*
	 ...
	 $option['is'] = type('is');
	 $option['AdLikeRole'] = type('AdLike.role');
